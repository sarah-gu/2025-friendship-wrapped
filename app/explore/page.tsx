...
export default async function ExplorePage() {
  ...
  // If fewer than 20 walls total, show all other walls (no random logic)
  // If 20 or more, take 20 random walls from others
  let wallsToShow: typeof otherWalls;
  if (allWrappeds.length < 20) {
    // Show all walls except user's wall
    wallsToShow = otherWalls;
  } else {
    // Shuffle and take 20 random walls from others
    const shuffled = [...otherWalls].sort(() => Math.random() - 0.5);
    wallsToShow = shuffled.slice(0, 20);
  }

  // Always put user's wall first if it exists, then show the selected walls
  const finalWallsToShow = userWall ? [userWall, ...wallsToShow] : wallsToShow;
    ...
    <div className="min-h-screen bg-black text-slate-100 font-sans relative">
      ...
      <main className="relative z-10 flex-1 max-w-7xl mx-auto w-full pt-28 md:pt-24 px-4 pb-32">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {finalWallsToShow.map((wrapped) => (
            <Link
              key={wrapped.id}
              href={
                wrapped.ownerId === currentUserId
                  ? `/dashboard`
                  : `/w/${wrapped.slug}`
              }
